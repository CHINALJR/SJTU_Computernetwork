...
class SRServerListener(EventMixin):
  ...
  def _handle_packet_msg(self, conn, vns_msg):
    ...
    core.cs640_srhandler.raiseEvent(SRPacketOut(pkt, out_port, self.swid))
...
class cs640_srhandler(EventMixin):
  ...
  def _handle_RouterInfo(self, event):
    log.debug("cs640_srhandler catch RouterInfo(info=%s, rtable=%s, swid=%s, dpid=%d)", event.info, event.rtable, event.swid, event.dpid)
    server = SRServerListener(event.swid, event.info, (self.host, (self.basePort + event.dpid)))
    self.servers[event.swid] = server
    log.debug("SRServerListener listening on %s" % server.listen_port)
...
def launch (transparent=False):
  ...
  """
  core.registerNew(cs640_srhandler)
